...
class EmbeddData:
    ...
    def build_knowledge_index(self):
        ...
        embedding = HuggingFaceEmbeddings(model_name=self.MODEL_NAME, model_kwargs={'device': self.CHIPSET})
        # print(embedding)

        print("4. constructing and persisting vector Database ... ")
        self.db = Chroma.from_documents(documents=chunks, embedding=embedding, persist_directory=self.DB_DIR)
